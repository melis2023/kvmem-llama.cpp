...
setlocal
REM ============================================================
REM  kvmem  CUDA 12.4  dll+exe  one-click build (self-contained)
REM  Supported GPUs: V100(70) / RTX20(75) / A100(80) / RTX30(86) / RTX40(89)
REM  NOTE: This build does NOT include sm_120 (RTX50). Use the
REM        13.4 script (build-cuda13.4-dll.bat) for local RTX 5060 Ti.
REM ============================================================

REM ---------- EDITABLE PARAMS (independent of the 13.4 script) ----------
set "SRC_DIR=%~dp0"
set "BUILD_DIR=%SRC_DIR%build-cuda12.4-dll"
set "NVCC=I:\llama\cuda temp\cuda-12.4-toolkit\bin\nvcc.exe"
set "CUDA_BIN=I:\llama\cuda temp\cuda-12.4-toolkit\bin"
set "CUDA_ARCH=70-real;75-real;80-real;86-real;89-real"
...
echo.
echo  ============================================
echo   kvmem CUDA 12.4 dll+exe build
echo   arch: %CUDA_ARCH%
echo   dir : %BUILD_DIR%
echo  ============================================
echo.
...
REM ---- check nvcc ----
if not exist "%NVCC%" (
    echo [ERROR] CUDA 12.4 nvcc not found: %NVCC%
